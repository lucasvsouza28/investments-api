...
namespace CaseBackend.Tests.Application.Query
{
    public class GetDirectTreasureInvestmentsHandlerTests
    {
        ...
        public void Setup()
        {
            ...
            _handler = Substitute.For<GetDirectTreasureInvestmentsHandler>(logger, restClient, options);
        }

        [Test]
        public async Task Should_Return_Response_With_No_Exceptions()
        {
            var response = await _handler.Handle(new GetDirectTreasureInvestmentsQuery(), CancellationToken.None);
